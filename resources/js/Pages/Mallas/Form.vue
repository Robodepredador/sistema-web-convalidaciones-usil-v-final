...
<template>
    <div class="w-full pb-16">
        ...
        <!-- Header Hero Banner -->
        <div class="mb-8 relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#1F3864] via-[#214378] to-[#2E75B6] p-8 sm:p-10 text-white shadow-xl">
            <!-- Decorative background elements -->
            <div class="absolute -top-24 -right-24 w-80 h-80 bg-white opacity-5 rounded-full blur-3xl mix-blend-overlay"></div>
            <div class="absolute bottom-0 right-1/3 w-64 h-64 bg-[#2E75B6] opacity-25 rounded-full blur-2xl"></div>
            ...
            <div class="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                ...
                <div class="flex items-center gap-3 w-full md:w-auto">
                    <Link href="/mallas/importar"
                          class="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 text-xs font-bold text-[#1F3864] shadow-md transition-all duration-200 hover:bg-blue-50 hover:scale-105">
        ...
        <form @submit.prevent="enviar" class="space-y-8">
            ...
            <div class="bg-white rounded-3xl border border-slate-200/70 shadow-sm p-6 sm:p-8 relative overflow-hidden">
                ...
                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                    <span class="flex h-7 w-7 items-center justify-center rounded-xl bg-blue-50 text-xs font-bold text-[#2E75B6]">
                        1
                    </span>
                    <div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200/70 shadow-sm p-6 sm:p-8 relative overflow-hidden">
                ...
                <div class="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="inline-flex items-center px-3 py-1 rounded-xl text-xs font-bold bg-blue-50 text-[#1F3864]">
                ...
                <div class="space-y-6">
                    <div v-for="(ciclo, ci) in form.ciclos" :key="ci"
                        ...
                        <div class="flex flex-wrap items-center justify-between gap-3 mb-4 pb-3 border-b border-slate-200/60">
                            <div class="flex items-center gap-3">
                                <div class="h-8 px-3 rounded-lg bg-[#1F3864] text-white flex items-center justify-center text-xs font-bold shadow-xs">
                                    Ciclo {{ ciclo.numero }}
                                </div>
                            ...
                            <div class="flex items-center gap-3">
                                <span class="text-xs font-semibold text-slate-500 bg-white px-2.5 py-1 rounded-lg border border-slate-200/60">
                                    <span class="font-bold text-[#2E75B6]">{{ creditosCiclo(ciclo) }}</span> créditos en este ciclo
                                </span>
                        ...
                        <div class="space-y-2.5">
                            ...
                            <div v-for="(curso, cj) in ciclo.cursos" :key="cj"
                                ...
                                <input v-model="curso.codigo" placeholder="Ej. INF101"
                                       class="w-full sm:w-32 rounded-lg border-slate-300 text-xs font-mono uppercase focus:border-[#2E75B6] focus:ring-[#2E75B6]" />
                                <input v-model="curso.nombre" placeholder="Nombre de la asignatura"
                                       class="flex-1 w-full rounded-lg border-slate-300 text-xs focus:border-[#2E75B6] focus:ring-[#2E75B6]" />
                                <input v-model="curso.creditos" type="number" step="0.5" min="0" placeholder="Créd."
                                       class="w-24 rounded-lg border-slate-300 text-xs text-center font-bold focus:border-[#2E75B6] focus:ring-[#2E75B6]" />
                        ...
                        <div class="mt-4 pt-2">
                            <button type="button" @click="agregarCurso(ci)"
                                    class="inline-flex items-center gap-1.5 text-xs font-bold text-[#2E75B6] hover:text-[#1F3864] bg-white px-3 py-1.5 rounded-xl border border-blue-100 hover:border-blue-200 transition-all shadow-2xs">
                ...
                <div class="mt-6">
                    <button type="button" @click="agregarCiclo"
                            class="group w-full py-4 rounded-2xl border-2 border-dashed border-slate-300 hover:border-[#2E75B6] hover:bg-blue-50/40 text-slate-600 hover:text-[#2E75B6] font-bold text-sm flex items-center justify-center gap-2 transition-all duration-200">
                        <div class="w-6 h-6 rounded-full bg-slate-100 group-hover:bg-[#2E75B6] group-hover:text-white flex items-center justify-center transition-colors">
            ...
            <div class="bg-white rounded-3xl border border-slate-200/70 p-5 shadow-sm flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-4">
                    <div class="hidden sm:block text-right text-xs text-slate-500">
                        ...
                        <span class="font-bold text-[#2E75B6]">{{ totalCreditos }} créditos</span>
                    </div>

                    <button type="submit" :disabled="form.processing"
                            class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#1F3864] to-[#2E75B6] px-7 py-3 text-sm font-bold text-white shadow-md hover:shadow-lg hover:scale-102 active:scale-98 transition-all duration-200 disabled:opacity-60">
