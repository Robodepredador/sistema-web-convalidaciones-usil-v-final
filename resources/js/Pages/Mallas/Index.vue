                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="relative z-20">
                        ...
                        <input v-model="filtro.anio" type="number" placeholder="Ej. 2026" min="2000" max="2100"
                               class="w-full rounded-lg border-slate-300 text-sm focus:border-[#2E75B6] focus:ring-[#2E75B6] transition-colors" />
                    </div>
                </div>
            ...
            <div class="p-4 sm:p-6">
                <div class="flex flex-wrap items-center justify-between gap-4 mb-4 px-2">
                    ...
                    <div v-if="mallas.data?.length" class="text-xs font-semibold text-slate-500 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200/60">
                        Mostrando <span class="text-[#1F3864]">{{ mallas.from }}</span>–<span class="text-[#1F3864]">{{ mallas.to }}</span> de <span class="text-[#1F3864]">{{ mallas.total }}</span> registros
                    </div>
                </div>
                            ...
                            <tr v-for="m in mallas.data" :key="m.id"
                                ...
                                <td class="px-5 py-4 rounded-l-2xl border-y border-l border-slate-100 group-hover:border-blue-100">
                                    <div class="flex items-start gap-3.5">
                                        <div class="mt-0.5 h-10 w-10 shrink-0 flex items-center justify-center rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 text-[#1F3864] ring-1 ring-inset ring-blue-100">
                                            <svg class="h-5 w-5 text-[#2E75B6]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.6">
                                                <path stroke-linecap="round" stroke-linejoin="round" d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342" />
                                            </svg>
                                        </div>
                                        <div>
                                            <div class="font-bold text-slate-800 text-sm sm:text-base group-hover:text-[#1F3864] transition-colors">
                                                {{ m.carrera }}
                                            </div>
                                ...
                                <td class="px-5 py-4 rounded-r-2xl border-y border-r border-slate-100 group-hover:border-blue-100 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        <Link :href="`/mallas/${m.id}`"
                                              class="inline-flex items-center gap-1.5 rounded-xl bg-[#1F3864] px-3.5 py-1.5 text-xs font-bold text-white shadow-xs transition-all duration-200 hover:bg-[#2E75B6] hover:scale-105"
                                        ...
                                        <Link :href="`/mallas/${m.id}/editar`"
                                              class="inline-flex items-center justify-center p-1.5 rounded-xl border border-slate-200 text-slate-600 hover:text-[#2E75B6] hover:border-[#2E75B6] hover:bg-blue-50/50 transition-colors"
                            ...
                            <tr v-if="!mallas.data?.length">
                                <td colspan="6" class="px-6 py-16 text-center rounded-2xl border border-slate-100 bg-slate-50/50">
                                    <div class="flex flex-col items-center justify-center max-w-md mx-auto">
                                        <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-blue-100/60 text-[#2E75B6]">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <Link href="/mallas/crear" class="inline-flex items-center gap-2 rounded-xl bg-[#1F3864] px-5 py-2.5 text-sm font-bold text-white shadow-md transition-all hover:bg-[#2E75B6]">
                    ...
                    <nav class="flex flex-wrap items-center gap-1.5">
                        <template v-for="(link, i) in mallas.links" :key="i">
                            <Link v-if="link.url" :href="link.url" preserve-scroll
                                  :class="link.active ? 'bg-[#1F3864] text-white font-bold shadow-xs' : 'bg-white text-slate-600 hover:bg-slate-100 border border-slate-200'"
